    ...
    >
      <div className={"flex flex-col space-y-2 mb-5"}>
        <div className={"flex w-full"}>
          ...
          <p className={"w-2/3"}>
            <ExpenseAmountView amount={String(expense.amount)} />
          </p>
        </div>
        <div className={"flex w-full"}>
        ...
        <div className={"flex w-full"}>
          ...
          <p className={"w-2/3"}>
            <DateView dateStr={new Date(parseInt(expense.time) * 1000).toISOString()} showTime />
          </p>
        </div>
        <div className={"flex w-full"}>
          <p className={"w-1/3"}>👨🏻‍💼 Card owner: </p>
          <p className={"w-2/3"}>
            {cardOwner?.name} {cardOwner?.surname}
          </p>
        </div>
        <div className={"flex w-full"}>
      ...
      {showSplit && (
        <>
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <InputNumber min={2} value={splitCount} onChange={handleSplitCountChange} size="small" />
            <Typography.Text className={"text-xs"} type={"danger"}>
              *first split is the original expense
            </Typography.Text>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {splitAmounts.map((amount, index) => (
              <div key={index} className="flex items-center gap-2">
                ...
                <InputNumber value={amount} min={0} onChange={(val) => updateAmount(index, val)} size="small" />
              </div>
            ))}
          </div>
