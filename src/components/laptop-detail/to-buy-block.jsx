  ...
  return (
    <Card bordered={false} hoverable={true} className={"w-2/4"}>
      <div className={"flex justify-between align-middle"}>
        ...
        <Button size={"small"} onClick={() => setAddToBuyItemActive(true)}>
      ...
      {laptop.toBuy.map((item, index) => {
        return (
          <div className={"flex align-middle justify-between mb-2"} key={item + index}>
            ...
            <div>
              <Button className={"bg-rose-400 text-white mr-1"} size={"small"} onClick={() => handleRemoveToBuy(index)}>
      ...
      {addToBuyItemActive && (
        <div className={"flex justify-between align-middle"}>
          <Input
            className={"mr-2"}
            value={newToBuyItem}
            onChange={(e) => setNewToBuyItem(e.target.value)}
            onPressEnter={handleNewToBuy}
          />
          <Button size={"small"} className={"bg-rose-400 text-white mr-1"} onClick={cancelAddNewItem}>
            <FontAwesomeIcon icon={faXmark} />{" "}
          </Button>
          <Button size={"small"} className={"bg-emerald-500 text-white"} onClick={handleNewToBuy}>
            <FontAwesomeIcon icon={faPlus} />{" "}
          </Button>
        </div>
      )}
