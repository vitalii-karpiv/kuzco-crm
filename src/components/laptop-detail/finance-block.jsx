...
export default function FinanceBlock({ laptop, setLaptop }) {
  ...
  return (
    <Card bordered={false} hoverable={true} className={"w-2/4 mr-3"}>
      ...
      <PriceInput
        label={"Limit price"}
        editMode={editLimitPrice}
        setEditMode={setEditLimitPrice}
        laptop={laptop}
        setLaptop={setLaptop}
        property={"limitPrice"}
        type={"warning"}
      ...
      <PriceInput
        label={"Sell price"}
        editMode={editSellPrice}
        setEditMode={setEditSellPrice}
        laptop={laptop}
        setLaptop={setLaptop}
        property={"sellPrice"}
        type={"success"}
    ...
    <Typography.Text className={"block p-2 my-5 rounded bg-slate-300"} onClick={() => setEditMode(true)}>
      ...
      {editMode ? (
        <InputNumber
          ...
          size={"small"}
          className={"w-56"}
          defaultValue={laptop[property]}
          onBlur={(event) => handlePriceUpdate(event.target.value)}
          onPressEnter={(event) => handlePriceUpdate(event.target.value)}
        />
      ) : (
        <Typography.Text className={"text-green-800"} keyboard strong>
          {showPrice(laptop[property])} грн
        </Typography.Text>
      )}
    </Typography.Text>
  );
