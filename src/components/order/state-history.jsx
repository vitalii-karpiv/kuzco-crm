    ...
    <Card bordered={false} hoverable={true} className={"w-2/4"}>
      ...
      <div className={"my-2"}>
        {order.stateHistory.reverse().map((state) => {
          return (
            <div key={state.timestamp} className={"bg-slate-300 p-3 rounded-lg my-3"}>
              {OrderManager.getOrderStateLabel(state.state)} <DateView dateStr={state.timestamp} showTime />{" "}
              {state.initiator}
            </div>
          );
        })}
