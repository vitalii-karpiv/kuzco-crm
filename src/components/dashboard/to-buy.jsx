        ...
        <Card className={"w-2/4 ml-2"} loading={loading}>
                        ...
                        {laptops.map(laptop => (
                            <li key={laptop.id}>
                                <Link to={`/laptops/laptopDetail/${laptop._id}`}>
                                    <Typography.Text><Typography.Text code>{laptop.code}</Typography.Text> {laptop.name}</Typography.Text>
                                </Link>
                                <ul className={"list-decimal list-inside space-y-2 ml-8 mt-2"}>
                                            ...
                                            {editing[laptop._id]?.[idx] !== undefined ? (
                                                <>
                                                    <Input
                                                        ...
                                                        onChange={e => setEditing(prev => ({...prev, [laptop._id]: {...(prev[laptop._id] || {}), [idx]: e.target.value}}))}
                                                        onBlur={() => handleEditToBuy(laptop, idx)}
                                                        onPressEnter={() => handleEditToBuy(laptop, idx)}
                                                        className="mr-2 w-40"
                                                    />
                                                    <Button size="small" icon={<FontAwesomeIcon icon={faXmark}/>} onClick={() => setEditing(prev => ({...prev, [laptop._id]: {...(prev[laptop._id] || {}), [idx]: undefined}}))} className="mr-1">
                                                        <Typography.Text type="secondary" style={{fontSize: 13}}>Cancel</Typography.Text>
                                                    </Button>
                                                    <Button size="small" onClick={() => handleEditToBuy(laptop, idx)} className="mr-1">
                                                        <Typography.Text type="secondary" style={{fontSize: 13}}>Save</Typography.Text>
                                                    </Button>
                                                </>
                                            ) : (
                                                <>
                                                    <Typography.Text className="mr-2">{toBuy}</Typography.Text>
                                                    <Button size="small" icon={<FontAwesomeIcon icon={faPen}/>} className="mr-1" onClick={() => setEditing(prev => ({...prev, [laptop._id]: {...(prev[laptop._id] || {}), [idx]: toBuy}}))}>
                                                        <Typography.Text type="secondary" style={{fontSize: 13}}>Edit</Typography.Text>
                                                    </Button>
                                                </>
                                            )}
                                            <Button size="small" icon={<FontAwesomeIcon icon={faCheck}/>} className="mr-1" onClick={() => handleBought(laptop, idx)}>
                                                <Typography.Text type="secondary" style={{fontSize: 13}}>Bought</Typography.Text>
                                            </Button>
                                            <Button size="small" icon={<FontAwesomeIcon icon={faXmark}/>} danger onClick={() => handleRemoveToBuy(laptop, idx)}>
                                                <Typography.Text type="secondary" style={{fontSize: 13}}>Remove</Typography.Text>
                                            </Button>
                                        </li>
                                    ))}
                                    <li className="flex items-center mt-2">
                                        ...
                                        <Button size="small" icon={<FontAwesomeIcon icon={faPlus}/>} onClick={() => handleAddToBuy(laptop)}>
                                            <Typography.Text type="secondary" style={{fontSize: 13}}>Add</Typography.Text>
                                        </Button>
                                    </li>
                                </ul>
                            </li>
            ...
            {modalOpen && modalLaptop &&
                <BuyStockModal
                    ...
                    onReload={() => { setModalOpen(false); fetchLaptops(); }}
                    id={modalLaptop._id}
                    index={modalIndex}
