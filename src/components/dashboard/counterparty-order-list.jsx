  ...
  return (
    <Card className={"w-2/4 ml-2 mt-1 min-h-60"} loading={isLoading}>
      ...
      <ol className={"list-disc list-inside space-y-2 overflow-y-auto max-h-[310px]"}>
        {orders.map((order) => (
          <li key={order.code}>
            <Link target={"_blank"} to={`/orders/orderDetail/${order._id}`} className={"bg-violet-50 rounded p-1"}>
              <Typography.Text code>{order.code}</Typography.Text> {order.name} <OrderStateTag state={order.state} />{" "}
