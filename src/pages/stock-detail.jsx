  ...
  return (
    <Card bordered={false} hoverable={true} className={"w-full mx-3"} title={stock.name}>
      <div className={"flex mb-2"}>
        <p className={""}>Code: </p>
        <Input className={"w-56 ml-2"} defaultValue={stock.code} size={"small"} readOnly={true} />
      </div>
      <div className={"flex mb-2"}>
        <p className={""}>Price: </p>
        <Input
        ...
        >
          {Object.values(StockManager.getStockStateMap()).map((state) => (
            <Select.Option key={state} value={state}>
              {StockManager.getStockStateLabel(state)}
            </Select.Option>
          ))}
        </Select>
      </div>
      ...
      <div className={"flex mb-2"}>
        ...
        >
          {laptopList.map((laptop) => (
            <Select.Option key={laptop._id} value={laptop._id}>
              {laptop.name}
            </Select.Option>
          ))}
        </Select>
        <Button
          ...
          shape={"circle"}
          size={"small"}
          onClick={() => navigate(`/laptops/laptopDetail/${laptop._id}`)}
        >
